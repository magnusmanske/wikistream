...
require_once __DIR__ . "/../scripts/SparqlRetryTrait.php";
require_once __DIR__ . "/../scripts/QuickStatementsBot.php";
...
class WikiStream implements ItemViewReader
{
	...
	// Bot-only tunables (PRE_1900_PD_PER_RUN, IA_*, P953_*, C2_*, C3_*)
	// moved to QuickStatementsBot. The remaining constants below are
	// still referenced by the ingest path on WikiStream itself.
	...
	protected ?QuickStatementsBot $bot = null;

	public function __construct(
		$config = null,
		$tfc = null,
		?HttpClientInterface $httpClient = null,
		?QuickStatementsBot $bot = null,
	) {
		if ($config == null) {
			die("Config not set");
		}
	...
	/**
	 * Lazily build the QuickStatements bot, sharing the same
	 * dependencies as this WikiStream. Tests can inject a custom bot
	 * (with overridden pushQuickStatements / loadWikidataItemList) via
	 * the constructor.
	 */
	protected function bot(): QuickStatementsBot
	{
		return $this->bot ??= new QuickStatementsBot(
			$this->config,
			$this->tfc,
			$this->db,
			$this->httpClient,
	...
	// sparqlRetried / sparqlBackoffSleep / $sparqlRetryBaseSleepUs are
	// now provided by SparqlRetryTrait so QuickStatementsBot can share
	// the same implementation (audits/STATUS.md P1.2).
	...
	// Bot methods (pushQuickStatements, import_*, annotate_*, parseP953Url,
	// parse_seconds) moved to QuickStatementsBot. Thin façade delegates
	// below preserve the old call sites (update.php and the dispatcher).

	public function import_ia_curated_films(): void { $this->bot()->import_ia_curated_films(); }

	public function import_p953_urls(): void { $this->bot()->import_p953_urls(); }

	/** Static façade — see QuickStatementsBot::parseP953Url. */
	public static function parseP953Url(string $url): ?array
	{
		return QuickStatementsBot::parseP953Url($url);
	}

	public function import_ia_curated_imdb_p724(): void { $this->bot()->import_ia_curated_imdb_p724(); }

	public function import_commons_pd_films_via_p180(): void { $this->bot()->import_commons_pd_films_via_p180(); }

	public function annotate_pre_1900_public_domain(): void { $this->bot()->annotate_pre_1900_public_domain(); }

	public function annotate_ia_movies(): void { $this->bot()->annotate_ia_movies(); }
